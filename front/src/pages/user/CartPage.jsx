...
const CartPage = () => {
  ...
  const handleCheckout = () => {
    ...
    if (!token) {
      ...
      // Nếu chưa đăng nhập → chuyển đến trang signin, kèm redirect về /checkout sau khi login
  ...
  const handleContinueShopping = () => {
    navigate('/'); // hoặc "/" tùy trang chủ bạn muốn
  };
  ...
  if (!cart) return <div className="p-6">Đang tải giỏ hàng...</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          cart.items.map(item => (
            ...
            >
              ...
              onClick={()=>{navigate(`/product/${item.product._id}`)}}>
                ...
                {item.variant && (
                  <p className="text-sm text-gray-500">Màu: {item.variant}</p>
                )}
      ...
      {/* Bên phải - chi tiết và hành động */}
