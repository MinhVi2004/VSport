...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axiosInstance.get("/api/order/admin"); // Yêu cầu isAdmin middleware
      const orderData = res.data;

      let total = orderData.length;
      let paid = orderData.filter((o) => o.isPaid).length;
      let unpaid = total - paid;
      let revenue = orderData.reduce((sum, o) => (o.isPaid ? sum + o.totalAmount : sum), 0);
      ...
      setOrders(orderData.slice(-5).reverse());
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Đã thanh toán" value={stats.paid} color="green" />
        <StatCard title="Chưa thanh toán" value={stats.unpaid} color="red" />
        <StatCard title="Tổng doanh thu" value={stats.revenue.toLocaleString()} prefix="₫" />
      </div>
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order._id} className="border-b hover:bg-gray-50">
                  <td className="p-2">{order._id.toUpperCase()}</td>
                  <td className="p-2">{order.address.fullName || "N/A"}</td>
